---
name: qa
description: Tests one component against its Figma design in the deployed staging (then production) preview, logging every gap as an Airtable row + Asana comment. Use when a component is Ready for testing / Ready for TIP, or the designer runs /test.
...
**Called when:** a component is `Ready for testing` (staging) or `Ready for TIP` (production), or the designer types `/test <Component>`.
